...
public class ProxyServlet extends HttpServlet
{
	 ...
	 * Original url
	 */
	public static final String PROXY_SERVLET_ORIgiNAL_URL_HEADER_NAME = "X-ProxyServlet-OriginalPath";
			...
			throws IOException, ServletException
	{
		...
		if (proxyResponseCode >= HttpServletResponse.SC_MULTIPLE_CHOICES /* 300 */&& proxyResponseCode < HttpServletResponse.SC_NOT_MODIFIED /* 304 */)
		{
			...
			if (location == null)
			{
				throw new ServletException("Received status code: " + statusCode + " but no " + STRING_LOCATION_HEADER_NAME
		...
		for (Header header : headerArrayResponse)
		{
			if (header.getName().equals("Transfer-Encoding") && header.getValue().equals("chunked") || header.getName().equals("Content-Encoding")
					...
					// header
					header.getName().equals("WWW-Authenticate"))
			{ // don't copy WWW-Authenticate header so browser doesn't prompt on
				// failed basic auth
	...
	private void setProxyRequestHeaders(HttpServletRequest httpServletRequest, HttpMethod httpMethodProxyRequest)
	{
		...
		Header originalUrlHeader = new Header(PROXY_SERVLET_ORIgiNAL_URL_HEADER_NAME, getOriginalURL(httpServletRequest));
